  ...
  <div class='userWrap'>
    <div class="wrap">
      <div class="user-center-wrap">
        ...
        <div class="item" @click="pushchange()">
          <span class="fl"><span class='iconfont icon-shezhi1'></span>推送设置</span>
          <span class='iconfont icon-quanjugengduojiantou-copy fr' ></span>
        </div>
        <div class="item"  @click='PushNews()'>
          <span class="fl"><span class='iconfont icon-kaiguan'></span>推送消息<i v-show="$store.state.user.warnFlg>0" class="red-point" style="margin-left: 5px;"></i>
          </span>
          <span class='iconfont icon-quanjugengduojiantou-copy fr'></span>
        </div>
        <div class="item  mb13"  @click='showFavorites()'>
        ...
        <div class="item mb13" @click="about()" >
          <span class="fl"><span class='iconfont icon-guanyu' ></span>关于</span>
          <span class='iconfont icon-quanjugengduojiantou-copy fr'  ></span>
        </div>
        <div class="item mb13"  @click="openPop('ee')">
...
//        version:version
    ...
    created(){
      ...
      axios.post('themeapp/themeCount', qs.stringify({'userId':this.$store.state.user.userId})).then(function (res) {
        ...
        if(this.$route.params.searchKeys == "anything"){
          _this.searchKeys=_this.$route.params.searchKeys;
          setTimeout(function(){
            _this.showFavorites();
          },10)
        }
        if(this.$route.params.searchKeys == "tuisongxiaoxi"){
          _this.searchKeys=_this.$route.params.searchKeys;
          setTimeout(function(){
            _this.PushNews();
          },10)
...
//    this.$refs.Favorites.fetchTheme();
    ...
    PushNews(){//推送消息
      //单击去掉小红点
      this.$store.commit('warnFlgClear');
