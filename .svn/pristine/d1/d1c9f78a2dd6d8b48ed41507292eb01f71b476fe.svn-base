...
.minheight{
  ...
  background-image:url('login-bg.png');
  background-repeat:no-repeat;
  background-size:cover;padding:0 1rem;
}
...
<template>
  <div class="signinWrapper">
    <div class="minheight">
	  ...
	  <mt-button class="signinBtn" type="danger" @click.native="resetPwd" size="large">提交</mt-button>
    </div>
    </div>
</template>
      ...
      passwordMsg:{
        get: function() {
          if (!this.ruleForm.password) {
            return '请输入密码'
          }else if(!/^[a-zA-Z0-9]*([a-zA-Z][0-9]|[0-9][a-zA-Z])[a-zA-Z0-9]*$/.test(this.ruleForm.password) || this.ruleForm.password.length < 6||this.ruleForm.password.length>12){
