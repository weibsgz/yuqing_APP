...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      submit() {
        if(document.getElementById('name').value.trim().length>10){
          Toast({
            message: '主题名称长度超长',
          });
          return;
...
//          console.log(this.noquerykeywords);
        ...
        axios.post(_this.urllink,qs.stringify(params)).then(function (res) {
          if(res.data.code==0){
            ...
            _this.$store.commit('reflashSet');//修改主题后刷新其它页面
            Toast({
              message: '提交成功',
            });
            //首次设置后，跳到主页
...
//        store.commit('increment')
      ...
      addKeywords:function () {
        ...
        if(document.getElementById('keyWordsItem').value.trim().length>30){
          Toast({
            message: '长度超长',
          });
          return;
        ...
        if(this.editem.keywords){
          if(this.editem.keywords.length>9){Toast({message: '最多输入10组',});return;}
          this.editem.keywords.push(document.getElementById('keyWordsItem').value)
        }else {
          if(this.keyWords.length>9){Toast({message: '最多输入10组',});return;}
      ...
      addnoquery:function () {
        ...
        if(document.getElementById('unkeyWords').value.trim().length>10){
          Toast({
            message: '长度超长',
          });
          return;
        ...
        if(this.editem.noquerykeywords){
          if(this.editem.noquerykeywords.length>14){Toast({message: '最多输入15组',});return;}
          this.editem.noquerykeywords.push(document.getElementById('unkeyWords').value)
        }else {
          if(this.noquerykeywords.length>14){Toast({message: '最多输入15组',});return;}
