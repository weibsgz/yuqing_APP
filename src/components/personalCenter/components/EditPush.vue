  ...
  <div class="subject-mangement">
    ...
    <div class="formlist-wrap">
      <div class="formlist">
        ...
        <div>
          <select id="selectedTheme">
            <template v-for="option in options">
              <option v-bind:value="option.themeId" v-if="option.themeId == editem.themeId" selected>
                {{ option.themeName }}
              </option>
              <option v-bind:value="option.themeId" v-else>
                {{ option.themeName }}
              </option>
            </template>
          </select>
        ...
        <div class="formlist">
          <span>手机号</span>
          <input type="text" name="typeinput2"  v-model='phones' placeholder='不可输入' :value='phones'  id='name' disabled="true">
        </div>
        <div class="formlist">
          <span>邮箱</span>
          <input type="email" name="typeinput2" v-model='mails' placeholder='不可输入' :value='mails' id='keyWords' disabled="true">
        </div>
...
//        themeName:'',
    ...
    methods: {
      init(theItem){
        ...
        this.sentimentType=theItem.sentimentType;
        this.mediaType=theItem.mediaType;
        var sn=theItem.freeTime?theItem.freeTime.split('-'):[];
        this.startTime=sn[0];
        this.endTime=sn[1];
        this.getoptions();//主题菜单
//        this.selectChange(theItem)
      },
      typesTab(item,ind){
        if(!!item.flag){
          item.flag=false
          document.getElementsByName("typeinput2")[ind].disabled=true;
          document.getElementsByName("typeinput2")[ind].placeholder='不可输入';
        }else {
          item.flag=true
          document.getElementsByName("typeinput2")[ind].disabled=false;
          document.getElementsByName("typeinput2")[ind].placeholder='请输入';
        }
        if(ind==0){
          this.phones=""
        }else{
...
//              console.log(v.itemCode,sen)
      ...
      submit() {
        ...
        let  _this=this;
        let selected=document.getElementById('selectedTheme');
        let index=selected.selectedIndex;
//        console.log(selected)
        let  parms={
          userId:this.$store.state.user.userId,
          themeId:selected.options[index].value,
          mails:this.mails,
          phones:this.phones,
          mediaType:this.mediaType,
          sentimentType:this.sentimentType,
          themeName:selected.options[index].text,
          freeTime:this.startTime+'-'+this.endTime
        }

        axios.post('/warnapp/editWarn',qs.stringify(parms)).then(function (res) {
          if(res.data.code==0){
