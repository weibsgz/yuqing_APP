...
<template>
  <div class="loginWrapper">
    ...
    <div class="forgetPwd"><router-link to="/resetPwd">忘记密码</router-link></div>
    ...
    <router-link to="/register"><mt-button  class="signinBtn">立即注册</mt-button></router-link>
      ...
      <mt-popup class="wh100" v-model="bpss.aa" popup-transition="popup-fade" :modal=false>
        <Mgedit :editem='editem' :themesList='themesList' :isStart="isStart" v-on:onChangePss="onChangePss"></Mgedit>
      </mt-popup>
  ...
  export default {
    ...
    created(){
        if(Boolean(localStorage.getItem('user'))){
          this.$router.replace({ path: '/yuqingdongtai' })
        }
    },
    components: {
      Mgedit
      ...
      passwordMsg:{
        get: function() {
          if (!this.form.password) {
            return '请输入密码'
          }else if(!/^[a-zA-Z0-9]*([a-zA-Z][0-9]|[0-9][a-zA-Z])[a-zA-Z0-9]*$/.test(this.form.password) || this.form.password.length < 6||this.form.password.length>12){
...
//          console.log(res.data)
         // var json = {};
          ...
          _this.form.warnFlg=res.data.warnFlg;
          _this.form.versionName=res.data.versionBean.versionCode;
          _this.form.reflash=[true,true];//设置刷新不刷新
...
//        _this.status = false
    ...
    edit(item) {
      ...
      this.bpss.aa=!this.bpss.aa;
//      this.$store.commit('increment')
      // this.$store.commit('setEditContent',item)
    },
      onChangePss(val){
        this.bpss[val] =!this.bpss[val];
      },
  }
  }
</script>
